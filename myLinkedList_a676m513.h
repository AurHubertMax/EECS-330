...
class MyLinkedList
{
    ...
    int theSize; // the number of elements that the linked list is currently
    holding
    ...
    // defined as proected as we don't expect it to be called directly by other
    classess
    ...
    current{nullptr} 
    {}
    // add const to the return type and make the return value by retrieve() 
    non-mutable
    ...
    // don't need to overload operator= and operator!= as they should behave 
    the same as in const_iterator
    };
    ...
    // insert x before itr; return iterator pointing to the newly inserted data 
    element
    ...
    Node *p = itr.current;
    theSize++;
    return (p -> prev = p -> prev -> next = new Node(std::move(x), p -> prev, 
    p));
    // code ends 
    }
    // delete the data element pointed by itr; return the iterator pointing to the 
    data element next to the one being deleted
    ...
    // delete the data elements from iterator "from" (inclusive) to iterator "to" 
    (exclusive)
    ...
    {
    DataType p = itr.current -> data;
    itr.current -> data = itr.current -> next -> data;
    itr.current -> next -> data = p;
