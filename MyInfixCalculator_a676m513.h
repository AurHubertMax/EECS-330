...
class MyInfixCalculator{
  ...
  public:
    ...
    double calculate(const std::string& s)
    {
        ...
        double temp = calPostfix(postfix);
        return temp;
  ...
  private:
    ...
    double calPostfix(const MyVector<std::string>& postfix_tokens) const
    {
        ...
        for (int i = 0; i < postfix_tokens.size(); ++i)
        {
            ...
            if (postfix_tokens[i].length() > 1) //push to stack if postfix token > 1
            {
            ...
            {
                std::string firstElem = stack.top();
                stack.pop();


                std::string secondElem = stack.top();
                stack.pop();

                stack.push(std::to_string(computeBinaryOperation(secondElem,firstElem,postfix_tokens[i])));
            }
        }
